...
2. Checks all baults for earned rewards (`mainLoop` function).
3. Values the earned reward as WBERA in staking-token terms.
...
3. The earned reward claimed as WBERA needs to be swapped for bault assets to return to the BountyHelper.
4. The script uses Enso protocol to achieve this.
5. The compoundors profit is sent to the configurable beneficiary in underlying asset tokens.
...
- `MIN_EARNED_REWARD_AMOUNT` - Minimum earned reward amount required before fetching quotes.
- `WRAPPER_SLIPPAGE_BPS` - Slippage for estimating WBERA reward value in staking-token terms.
